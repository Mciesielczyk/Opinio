...
class FriendsRepository extends Repository
{
    ...
    public function getFriendsList(int $user_id): array
    {
        $stmt = $this->database->connect()->prepare('
            SELECT u.id, u.name, u.surname, u.profile_picture, u.description, u.location
            FROM users u
            INNER JOIN friends f ON (f.user_id_1 = u.id OR f.user_id_2 = u.id)
            WHERE (f.user_id_1 = :user_id OR f.user_id_2 = :user_id) AND u.id != :user_id AND f.status = \'accepted\'
        ');
